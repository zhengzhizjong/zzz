  ...
  <div class="store-ranking">
      ...
      <el-form :inline="true" :model="filterForm" class="filter-form">
        ...
        <el-form-item label="排行维度">
          <el-radio-group v-model="filterForm.dimension" @change="handleSearch">
            ...
            <el-radio-button label="customer">客流</el-radio-button>
    ...
    <el-card shadow="never" class="table-card">
      <el-table :data="tableData" v-loading="loading" stripe border>
        ...
        <el-table-column label="营收" width="130" align="center" v-if="filterForm.dimension === 'revenue'">
          <template #default="{ row }">
            ¥{{ row.revenue?.toFixed(2) ?? '0.00' }}
          </template>
        </el-table-column>
        <el-table-column label="客流" width="100" align="center" v-if="filterForm.dimension === 'customer'">
          <template #default="{ row }">
            {{ row.customerCount ?? 0 }}
          </template>
        </el-table-column>
...
import { getStoreRanking, type StoreRankingItem } from '@/api/data/ranking'

const loading = ref(false)
const tableData = ref<StoreRankingItem[]>([])
...
function getChangeValue(row: StoreRankingItem): number {
  if (filterForm.dimension === 'revenue') return row.revenueChange ?? 0
  if (filterForm.dimension === 'customer') return row.customerChange ?? 0
  return row.ratingChange ?? 0
}
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getStoreRanking({
